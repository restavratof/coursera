...
for ct in CTS:
    mess_xor = xor(target.encode(), ct.encode())
    # print(f'result : {mess_xor}')
    # for i, bit in enumerate(mess_xor):
    #     print(f'{i} - {bit}')
...
for ch1 in all_list:
    # for ch2 in all_list:
        result_dict[xor(ch1.encode(), space.encode())] = ch1
...
for ct in CTS:
    mess_xor = xor(target.encode(), ct.encode())
    print(f'result : {mess_xor}')
    print(f'result : {mess_xor.hex()}')
    # for i, bit in enumerate(mess_xor):
    #     print(f'{i} - {bit}')
        # if (result_dict[bin(bit)]):
            # print(f'{i} - {bit}')
            # print('OK')
...
print()
mess_xor = xor(target.encode(), key.encode())
print(f'1 : {mess_xor}')
print(f'2 : {mess_xor.decode()}')
print(f'3 : {mess_xor.decode("utf-8")}')
